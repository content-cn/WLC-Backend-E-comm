...
exports.getUerDetails= async (req, res) => {
    try {
      // Find the user details by ID and populate 'profileData' with 'name' and 'email' from the User model
      const userDetails = await UserDetails.findById(req.params.id).populate('profileData', 'name email');
...
exports.postUserDetails = async (req, res) => {
    try {
      ...
      if (!existingUser) {
        return res.status(404).json({ message: 'User not found in the User model' });
      }
      ...
      res.status(201).json({ message: 'User details added successfully', data: savedUserDetails });
    } catch (error) {
